...
function start_container() {
    ...
    # Determine authentication mode from settings.json (generated by apikey commands)
    local auth_mode="OAuth"
    local auth_detail=""
    if [[ -f "$GBOX_CLAUDE_DIR/settings.json" ]]; then
        local api_key_from_settings
        api_key_from_settings=$(jq -r '.env.ANTHROPIC_AUTH_TOKEN // ""' "$GBOX_CLAUDE_DIR/settings.json" 2>/dev/null || echo "")
        if [[ -n "$api_key_from_settings" ]]; then
            auth_mode="API Key"
            # Get provider name from api-providers.json
            if [[ -f "$GBOX_CLAUDE_DIR/api-providers.json" ]]; then
                local provider_name
                provider_name=$(jq -r '.default // ""' "$GBOX_CLAUDE_DIR/api-providers.json" 2>/dev/null || echo "")
                if [[ -n "$provider_name" ]]; then
                    auth_detail=" (provider: ${provider_name})"
                fi
            fi
        fi
    fi
    ...
    if (( quiet_mode == 0 )); then
        ...
        echo -e "  Authentication: ${BLUE}${auth_mode}${auth_detail}${NC}"
